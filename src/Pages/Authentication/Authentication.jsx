...
const Authentication = () => {
  ...
  const { loginWithEmail, signUpWithEmail, logOut } = useContext(AuthContext);
  ...
  const handleLogin = (event) => {
    ...
    axios
      .get(
        `https://e-commerce-server-side-eosin.vercel.app/eachUsers?email=${email}`
      )
      .then((result) => {
        if (result.data.number !== number) {
          ...
          loginWithEmail(email, password)
            .then((result) => {
              console.log(result);
              Swal.fire("Congratulations", "Logged in successfull", "success");
              form.reset();
              navigate("/");
            })
            .catch((error) => {
  ...
  const handleSignUp = (event) => {
    ...
    signUpWithEmail(email, password)
      .then((result) => {
        ...
        axios
          .post(
            "https://e-commerce-server-side-eosin.vercel.app/users",
            newUser
          )
          .then((data) => {
            if (data.data.insertedId) {
              Swal.fire("Congratulation", "Now login to continue", "success");
              form.reset();
              logOut();
            }
          });
  ...
  return (
    <div className="relative">
      <Card className="lg:absolute mx-0 m-5 p-5 space-y-2">
        <h2 className="text-2xl font-bold">Admin Login-</h2>
        <h4>Email: ecommerce@gmail.com</h4>
        <h4>Number: 01751779342</h4>
        <h4>Password: ECommerce</h4>
      </Card>
      <Card className="w-[99%] md:w-full max-w-md mx-auto my-6">
        <CardHeader
          color="gray"
          floated={false}
          shadow={false}
                ...
                <Card color="transparent" shadow={false}>
                  ...
                  >
                    ...
                    {!loading ? (
                      ...
                      <div className="w-full bg-cyan-500 flex justify-center py-2 rounded-md text-white text-lg hover:bg-cyan-600">
                        <Spinner color="blue" />
                      </div>
                    )}
                  </form>
